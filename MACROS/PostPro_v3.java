...
public class PostPro_v3 extends StarMacro {
...
public void execute() {
    ...
    String expath = sessionDir.replace("RUN", "POST") + sep;
	...
	String baselinePath = expath.replace(simname.substring(0,9), baseline); //DO NOT CHANGE
	String baselineSBD = simname.replace(simname.substring(0,9), baseline); //DO NOT CHANGE
	...
	//Run desired methods
	sim.println("Running post processing routines...\n");
	outputForces(sim, expath, simname);
	outputSurfVis(sim, expath, simname, doSurfCP, doSurfLIC, hardx, hardy);
	Slices(sim, expath, simname, "X", xmin, xmax, xstep, doXLIC, doXCP, doXCPT, hardx, hardy);
	Slices(sim, expath, simname, "Y", ymin, 0, ystep, doYLIC, doYCP, doYCPT, hardx, hardy);
	Slices(sim, expath, simname, "Z", zmin, zmax, zstep, doZLIC, doZCP, doZCPT, hardx, hardy);
	exportSurfCP(sim, expath, simname);

	if(doDelta == 1){
		DeltaCP(sim, expath, simname, baselinePath, baselineSBD, hardx, hardy);	
	}

	if(exportGBMR == 1){
		doGBMR(sim,expath,simname);
	}
...
private void outputForces(Simulation sim, String exportpath, String folder) {
	...
	sim.println("Outputting coefficients plot.");
    coeffs.export(resolvePath(exportpath + folder + "_Coefficients.csv"), ",");

    sim.println("Outputting averaged reports.");
...
private void outputSurfVis(Simulation sim, String exportpath, String folder, int doSurfCP, int doSurfLIC, int xsize, int ysize) {
    ...
    sim.println("Outputting surface visualization plots...");
...
private void Slices(Simulation sim, String exportpath, String folder, String direction, int minbound, int maxbound, int step, int doLIC, int doCP, int doCPT, int xsize, int ysize) {
	...
	sim.println("Outputting " + direction + " slices...");
	if (doLIC == 1){
		...
		for(int i=0; i<steps; i++){
			...
			sim.println("Outputting slice: LIC, " + direction + ", Position: " + (int)position + "mm");
	...
	if (doCP == 1){
		...
		for(int i=0; i<steps; i++){
			...
			sim.println("Outputting slice: CP, " + direction + ", Position: " + (int)position + "mm");
	...
	if (doCPT == 1){
		...
		for(int i=0; i<steps; i++){
			...
			sim.println("Outputting slice: CpT, " + direction + ", Position: " + (int)position + "mm");
...
private void exportSurfCP(Simulation sim, String exportpath, String folder){
    ...
    sim.println("Exporting surface SBD file...");
...
private void DeltaCP(Simulation sim, String exportpath, String folder, String basepath, String baseline, int xsize, int ysize){
    ...
    sim.println("Running delta plots...");
    for (int i = 0; i < orient.length; i++){
		...
		sim.println("Outputting delta surface: " + names[i]);
