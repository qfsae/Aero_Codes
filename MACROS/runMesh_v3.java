...
public class runMesh_v3 extends StarMacro {
...
private void partImporting(Simulation sim, String part){
    ...
    String importpath = sim.getSessionDir().split("RUN")[0] + "IMPORT" + sep;
...
private void UpdateParams(Simulation sim, int straight){
    ...
    GROUND_X_X = GROUND_X_X/Math.sqrt(Math.pow(wheelbase,2) + Math.pow(-RRH_Off + FRH_Off,2));
    GROUND_X_Z = GROUND_X_Z/Math.sqrt(Math.pow(wheelbase,2) + Math.pow(-RRH_Off + FRH_Off,2));

    GROUND_Y_Y = GROUND_Y_Y/Math.sqrt(Math.pow(0.1,2) + Math.pow(-0.1*Math.sin(roll_rad),2));
    GROUND_Y_Z = GROUND_Y_Z/Math.sqrt(Math.pow(0.1,2) + Math.pow(-0.1*Math.sin(roll_rad),2));
    ...
    ScalarGlobalParameter WB = ((ScalarGlobalParameter) sim.get(GlobalParameterManager.class).getObject("wheelbase"));

    WB.getQuantity().setValue(wheelbase);
